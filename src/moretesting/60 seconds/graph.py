...
def serial():
    ...
    for threadNum in threads:
        for popNum in popSize:
            for city in cities:
                file = open("openmp-"+str(threadNum)+"-"+str(popNum)+"-"+str(city)+"-60.txt")
                data = file.readlines()
                file.close()
                count = 1.0
                X = []
                ...
                plt.plot(X,y)
                minP = min(y)
                file = open("serial-"+str(popNum)+"-"+str(city)+"-60.txt")
                data = file.readlines()
                file.close()
                temp = count
                count = 1.0
                ...
                plt.title("Average fitness over 60 seconds for"+ str(city)+ "cities")
